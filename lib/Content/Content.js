...
class Content{
	 ...
	 *   @param { string } Options.type - The type of content
	...
	async getContentType( Opts, cb ){
		...
		if( typeof Opts.type !== 'string' ){
			...
			try{
				let uri = await MicroLib.renderEndpoint( this.Endpoints.getContentType, { type: Opts.type } );
	...
	async getContentById( Opts, cb ){
		if( !parseInt( Opts.id ) ){
			...
			try{
				let uri = await MicroLib.renderEndpoint( this.Endpoints.getContentById, { id: Opts.id, locale: Opts.locale } );
				let resp = await this._get( uri );
				cb( resp, false );
			} catch(e){
				cb( false, e );
	...
	async getContentByTagAndType( Opts, cb ){
		if( typeof Opts.locale !== 'string' ){
			cb( false, new MicroLib.TypeError( { varName: 'Options.locale', variable: Opts.locale, expected: 'string' } ) );
		} else if ( typeof Opts.tag !== 'string' ){
			cb( false, new MicroLib.TypeError( { varName: 'Options.tag', variable: Opts.tag, expected: 'string' } ) );
		} else if ( typeof Opts.type !== 'string' ){
			cb( false, new MicroLib.TypeError( { varName: 'Options.type', variable: Opts.type, expected: 'string' } ) );
		} else {
			try{
				let uri = await MicroLib.renderEndpoint( this.Endpoints.getContentByTagAndType, { locale : Opts.locale, tag : Opts.tag, type : Opts.type } );
				let resp = await this._get( uri );
				cb( resp, false );
			}catch( e ){
				cb( false, e );
	 ...
	 * };
	 *
	 * Content.searchContentWithText( Options , ( Response, err ) => {
	 *     if( err !== false ) {
	 *         // Error handling
	 *     } else {
     *         console.log( Response ); // Do something with the API response
	...
	async searchContentWithText( Opts, cb ){
		if( typeof Opts.locale !== 'string' ){
			...
			cb( false, new MicroLib.TypeError( { varName : "Options.cType", variable : Opts.cType, expected : 'string' } ) );
		} else if( ! parseInt( Opts.currentPage ) ){
			cb( false, new MicroLib.TypeError( { varName : "Options.currentPage", variable : Opts.currentPage, expected : 'number-like' } ) );
		} else if( typeof Opts.searchText !== 'string' ){
			cb( false, new MicroLib.TypeError( { varName : "Options.searchText", variable : Opts.searchText, expected : 'string' } ) );
		} else if( typeof Opts.tag !== 'string' ){
			cb( false, new MicroLib.TypeError( { varName : "Options.tag", variable : Opts.tag, expected : 'string' } ) );
		} else {
			try{
				...
				let uri  = await MicroLib.renderEndpoint( this.Endpoints.searchContentWithText, PathParams, QueryStrings);
				let resp = await this._get( uri );
				cb( resp, false );
	...
	async searchContentByTagAndType( Opts, cb ){
		if( typeof Opts.locale !== 'string' ){
			...
			try{
				...
				let uri = await MicroLib.renderEndpoint( this.Endpoints.searchContentByTagAndType, PathParams, QueryStrings);
				let resp = await this._get( uri );
			}catch( e ){ cb( false, e ); }
		}
	}
			...
			Https.get( this.Endpoints.rootPath + endpoint, { headers: headers }, res => { 
				...
				res.on('end', () => {
					try{
						...
						resolve( data );
					} catch(e) { reject (new MicroLib.ApiError( data ) ) };
				} );
			} );
		} );
	}
