...
class OAuth{
	...
	async generateAuthUri( cb ){		
		try{
			let uri = await MicroLib.renderEndpoint( this.Endpoints.authorization, {}, {
				response_type : 'code',
				client_id     : this.ApiCreds.clientId
			} );
	 ...
	 *     @property { string } Options.refreshToken - The refresh token you were given with your access token
	 * @param { ApiCallback } cb - The function to be called when the API call has completed
	 */
	async refreshAccessToken( Opts, cb ){
		try{			
			let response = await this._post( {
				path : this.Endpoints.refresh,
				PostData : {
