...
class Forum{
	 ...
	 * @param { Object } Options - The parameters required to complete this API call
	 *   @param{ number-like } Options.childPostId - 
	 ...
	 * @param { Object } Options - The parameters required to complete this request
	 *   @param { number-like } Options.childPostId - The post id that you want the parent for.
	 *   @param { boolean } [Options.showBanned=false] - Whether or not to include banned posts in the response
	 * @returns { Promise }
	 */
	async getPostAndParent( Opts ){	
	 ...
	 * @returns { Promise }
	 */
	async getPostAndParentAwaitingApproval( Opts ){	
		Opts.showBanned = ( typeof Opts.showBanned !== 'boolean' ) ? false : Opts.showBanned;
		return MicroLib.renderEndpoint( this.Endpoints.getPostAndParentAwaitingApproval, { childPostId: Opts.childPostId }, { showBanned : Opts.showBanned } )
		  .then( uri => this._get( uri ) );
	}
	 ...
	 * @returns { Promise }
	 */
	async joinFireteamThread( Opts ){
		return MicroLib.renderEndpoint( this.Endpoints.joinFireteamThread, { topicId : Opts.topicId, oAuth : Opts.oAuth } )
		  .then( uri => this._post( {
	...
	_post( Opts ){
		return new Promise( ( resolve, reject ) => {
			...
			Opts.PostData = (typeof Opts.PostData == 'object' ) ? QueryString.stringify( Opts.PostData ) : "";
