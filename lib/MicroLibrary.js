...
async function enumLookup( key, Table ){
	debug( "enumLookup( " + key +", " + JSON.stringify( Table ) + " )" );
	return new Promise( (resolve, reject) => {
		...
		if( typeOf !== 'number' && typeOf !== 'string' ){
			debug( "\tREJECTED: enumLookup( " + key +", " + JSON.stringify( Table ) + " )" );
			...
			if( typeOf == 'string' ){
				debug("\tRESOLVED: enumLookup( " + key +", " + JSON.stringify( Table ) + " )" );
				debug( "\t=====> " + Table[key] );
				resolve ( Table[key] );
			} else {
				debug( "\tRESOLVED: enumLookup( " + key + " )" );
				debug( "\t=====> " + key);
				resolve ( key );
			}
		}
...
async function renderEndpoint( uri, PathParams = {}, QueryStrings = null ){
	...
	debug( "\tPathParams : " + JSON.stringify( PathParams ) );
	debug( "\tQueryStrings : " + JSON.stringify( QueryStrings ) );
	return new Promise( ( resolve, reject) => {
...
function mapEnumSync( Obj ){
	debug( "mapEnumSync( " + JSON.stringify( Obj ) + " )" );
	let rtrn = Obj;
	Object.keys( Obj ).forEach( key => {
		rtrn[ Obj[ key ] ] = key;
	});
	debug( "\tmapped to " + JSON.stringify( rtrn ) );
	return rtrn;
}
...
class Request{
	...
	async get( uri, oAuth = false ){
		...
		return new Promise( ( resolve, reject ) => {
			...
			Https.get( uri, { headers: headers }, res => { 
				...
				res.on('end', () => {
					try{
						debug( "\tAttempting to parse response..." )
	...
	async post( uri, PostData, oAuth = false ){
		...
		return new Promise( ( resolve, reject ) => {
			...
			debug( "headers are: " + JSON.stringify( headers ) );
			debug( "======> Request sent" );
			...
			// Capture the response
				...
				Response.on( 'end', () => {
					debug( "Entire response received" );
					try{
						data = JSON.parse( data );
					} catch(e) {
						debug( data );
