...
const libInfo = JSON.parse( Fs.readFileSync( projectRoot + '/package.json' ) );
...
function defaultUserAgent(){
	return "node-bungie-lib/" + libInfo.version + " AppId/null (+https://www.tylermeador.com/docs/node-bungie-lib/" + libInfo.version + ";no-contact-email)";
}

/**
 * Generates a User-Agent string in the Bungie-recommended form
 * @param { Object } Options - The data requried to generate a UA string
 *   @param { string } [Options.appName="N/A"] - The name of your companion application
 *   @param { string } [Options.version="N/A"] - The version of your companion application
 *   @param { string } [Options.appId="N/A"] - The AppId that was givent to you when you created your companion application on Bungie.net
 *   @param { string } [Options.appSite="N/A"] - The website for your project, if any
 *   @param { string } [Options.email="N/A"] - The contact email for the author of your companion app
 */
function generateUA( Opts ){
	Opts.appName = defaults( Opts.appName );
	Opts.appId   = defaults( Opts.appId );
	Opts.appSite = defaults( Opts.appSite );
	Opts.email   = defaults( Opts.email );
	return Opts.appName + '/' + libInfo.version + " AppId/" + Opts.appId + " (+" + Opts.appSite + ';' + Opts.email + ')';
	
	function defaults( x ){
		return typeof x !== 'string' ? "N/A" : x;
	}
}
...
class Request {
		...
		return new Promise( ( resolve, reject ) => {
			...
			Https.get( uri, { headers }, res => {
				...
				res.on('end', () => {
					...
					if( res.headers['content-type'] === 'application/json' &&  data.ErrorCode === 1 ){
						resolve( data );
					} else {
						reject( data );
					}
	 ...
	 * @@returns { Promise }
	 */
	async post( uri, PostData, oAuth = false, progressCallBack = function(){} ){
			...
			// Capture the response
				...
				Response.on( 'end', () => {
					...
					try{
						...
						rDebug( data );
						reject( new Error ( 'Unauthorized: Access is denied due to invalid credentials' ) );
					}
