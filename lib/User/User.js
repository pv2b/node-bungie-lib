...
class User{
	...
	async getMembershipDataById( Opts, cb ){
		...
		if( typeof Opts.membershipId !== 'string'){
			cb( false, new MicroLib.TypeError( { varName: 'Options.membershipId', variable: Opts.membershipId, expected: 'string' } ) );
		}else if( typeof Opts.membershipType !== 'string'){
			cb( false, new MicroLib.TypeError( { varName: 'Options.membershipType', variable: Opts.membershipType, expected: 'string' } ) );
		}else{
