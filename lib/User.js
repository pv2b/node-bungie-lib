...
module.exports = class User{
	...
	async getBungieNetUserById( Opts, cb ){
		if( ! parseInt( Opts.id ) ){
			cb( false, new Error( "Options.id expected to be int, got " + typeof Opts.id ));
		} else {
	...
	async getMembershipDataById( Opts, cb ){
		...
		if( typeof Opts.id !== 'string'){
			cb( false, new Error( 'Opts.id expected to be string, got ' + typeof Opts.id) );
		}else if( typeof Opts.type !== 'string'){
			cb( false, new Error( 'Opts.type expected to be string, got ' + typeof Opts.type) );
		}else{
			// Try to generate a response
			try{
				let uri  = await this.render( this.Endpoints.getMembershipDataById, { membershipId: Opts.id, membershipType: Opts.type } )
				let resp = await this._get( uri );
