...
class GroupV2 {
	...
	getGroupOptionalConversations( groupId ){
		return MicroLib.renderEndpoint( this.Endpoints.getGroupOptionalConversations, { groupId } )
			.then( endpoint => Request.get( this.Endpoints.rootPath + endpoint ) );
	}
	 ...
	 *   @param { module:GroupV2/Enum~bungieMembershipType } Options.platformMembershipType - 
	 * @param { oAuth } oAuth - Your oAuth tokens
	 */ 
	createGroup( Opts, oAuth ){
		let enums = [
			MicroLib.enumLookup( Opts.membershipOption, this.Enums.membershipOption );
			MicroLib.enumLookup( Opts.chatSecuritySetting, this.Enums.chatSecuritySetting );
			MicroLib.enumLookup( Opts.groupHomepage, this.Enums.groupHomepage );
			MicroLib.enumLookup( Opts.platformMembershipType, this.Enums.bungieMembershipType );
		];
